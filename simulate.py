...
def make_transparent_color(ntimes, fraction):
    ...
    color = jnp.array(mpl.colors.to_rgba(
        mpl.cm.gist_ncar(fraction)))[np.newaxis, :]
...
class SimulationDataset(object):
    ...
    def _init_(self, sim='r2', n=5, dim=2,
    ...
    def simulate(self, ns, key=0):
        ...
        def make_sim(key):
            if sim in ['string', 'string_ball']:
                ...
                x0 = random.normal(key, (n, total_dim))
                # all masses set to positive
                x0 = x0.at[..., -1].set(np.exp(x0[..., -1]))
                if sim in ['charge', 'superposition']:
                    # charge is 1 or -1
                    x0 = x0.at[..., -2].set(np.sign(x0[..., -2]))
        ...
        if not animate:
            if sim in ['string', 'string_ball']:
                ...
                for j in range(n):
                    ...
                    if plot_size:
                        plt.scatter(
                            x_times[:, j, 0], x_times[:, j, 1], color=rgba, s=3*masses[:, j]*s_size)
                    else:
                        plt.scatter(
            ...
            for t_idx in range(d_idx, len(times), d_idx):
                ...
                for j in range(n):
                    ...
                    if plot_size:
                        plt.scatter(
                            cx_times[:, j, 0], cx_times[:, j, 1], color=rgba, s=3*masses[:, j])
                    else:
                        plt.scatter(
